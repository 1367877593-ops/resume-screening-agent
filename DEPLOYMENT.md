...
- `DEMO_MODE = false`
- `PERSIST_RUNS = false` —— 见下一节，公网部署必须关掉
...
本地运行时 `PERSIST_RUNS=1`，每次运行的完整结果会写进 SQLite，方便历史回看和
跨批次查询。**公网部署必须设为 `false`**，原因有三个：

1. 落库的 payload 里包含**简历原文**，也就是别人的姓名、学校、联系方式。
2. 所有访问者共用同一个进程、同一份 `app.db`，当前没有按用户隔离。
3. Community Cloud 的文件系统是临时的，应用重启即清空 —— 这份库既不安全，
   也根本留不住。

关掉之后界面功能完全不受影响，只是结果不落盘，关闭页面即清除；侧边栏会显示
「本次结果不落盘」。访问口令保护的是入口，不能替代这一条。
...
4. 侧边栏显示“本次结果不落盘”，确认 `PERSIST_RUNS=false` 已生效；
5. 上传一份非敏感测试简历可完成真实分析。
