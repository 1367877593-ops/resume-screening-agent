...
> 情况下完整展示 JD 拆解、两份简历提取与排序、匹配依据、10 道题、3 个追问、Checker
> 发现问题并修订一轮、三人格盲评抓出一道无区分度的题，以及 SQLite 持久化和调用链。
> 未实现的部分（跨运行反思飞轮、LLM 语义校验）在「已知局限」里逐条写明。
...
2. 补齐后的题里有一道「服务突发超时怎么分层排查」—— 三人格盲评发现背题党也能答得不错，
   判定 `Q_NO_DISCRIMINATION`，题目页会把那一格标红。
...
| Checker Agent | 确定性规则校验、统一 Gate、最多两轮定向修订与熔断转人工 |
...
计划把 Checker 的问题沉淀为经验库，按岗位类型与 `issue_code` 检索注入。当前只做到单次运行内的
check/revise 闭环，跨运行的经验检索没有实现 —— 不把规划写进能力表。
...
| 语义一致性 | `SEM_REASON_CONTRADICTS_EVIDENCE` | llm | 前四类判不了时才调 LLM（未实现） |
...
目前没有任何一条问题是靠 LLM 判出来的 —— 数量、schema、算术、字符串匹配、相似度
全部由确定性规则完成，题目质量由三分对照完成。这个数字不藏：如果哪天它倒过来，
说明这套校验的可信度需要重新评估。
...
**三人格盲评诊断**（内置样例的 10 道题）：好题 9 道（90%）、无区分度 1 道（10%）。
被标出的那道不需要人工逐条看。

**其余**：

- `make test`：**150 项全部通过**，覆盖 Harness、缓存原子写与并发、trace 归属、Gate/修订循环、
  代码算分、排序、SQLite、API 序列化、PDF/Word 解析、三人格信息隔离与真值表边界、
  连接检查的报错翻译，以及内置 Demo 端到端回放。CI 在 Python 3.9 与 3.12 上各跑一遍。
- Demo：2 份样例简历，李明 95.0 分推进，王芳因三项硬性要求不满足一票否决淘汰。
- 回放调用：12 个结构化缓存条目，命中率 100%，全程不初始化真实模型客户端。
...
- **三人格盲评本身仍是模型在评模型**，只是把它约束成了信息量对照实验：判定看三个分数的相对关系而非绝对分，人格拿不到评分标准，阅卷官拿不到身份。这降低了循环论证的程度，没有消除它。诊断阈值目前按经验取值，没有人工标注集校准，可能对当前样例过拟合。
- 跨运行的反思飞轮未实现，`checker/semantic.py`（LLM 语义校验）未实现。
