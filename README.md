...
> **当前状态**：端到端闭环已完成。内置 Demo 可在无 API Key、无网络请求的
> 情况下完整展示 JD 拆解、三份简历的提取与排序（推进 / 待定 / 淘汰各一）、匹配依据、
> 每人 10 道题与 3 个追问（每道题都写明考察点与出题原因）、Checker 发现问题并修订一轮，
...
2. 陈涛 R5 的判定理由声称「有系统的多版迭代经验」，而它引用的那句原文恰恰写着
   「没有做过多版对比或效果评估」—— 引用逐字存在，归因规则查不出来，
   只有语义校验能看出证据撑不起结论。

Demo 展示的是「发现问题 → 反馈 → 修订 → 复检」的闭环在工作，
而不是一份预先准备好的完美报告。
...
| 试题生成 | 生成 ≥10 道面试题，每道题写明**考察点**与**出题原因**，含难度与分档评分标准 |
...
flowchart TD
    ...
    I -->|REJECT| N["结构化决策建议"]
    I -->|ADVANCE / HOLD，按需| J["≥10 道试题 + 3-5 追问<br/>（含考察点与出题原因）"]
    J --> L{"Checker：题目校验"}
    L -->|FAIL| M["修订 → 增量重跑"]
    M --> L
...
一道面试题如果只有题干，面试官拿到手无从判断该不该问。因此每道题都必须同时给出两项，
它们回答的是不同的问题：

| 字段 | 回答什么 | 例子 |
|---|---|---|
| `skill_point` | 考察什么能力 | 效果归因 |
| `rationale` | 为什么问**这位**候选人 | 简历把 23 个百分点的提升记在个人名下，却没写归因方法；R5 正是匹配阶段判为 PARTIAL 的存疑项，要确认这是他定位出来的还是团队的共同结论 |

关键在于 `rationale` **必须落到具体的人**：指向简历里说不清楚的地方、匹配结论里的存疑项，
或某条要求缺少证据。「考察候选人的工程能力」这种换谁都成立的话不算 —— 那是考察点，不是原因。
出题 prompt 里给了正反例，schema 里两个字段并列存放，界面上分两行展示。

**没有解决什么**：`rationale` 写得够不够具体，靠模型自觉，当前只有非空校验。
真要兜底得让它引用简历原文或存疑项编号，那样才能像 evidence 一样被确定性规则核对。
...
**它没有解决什么**：跑三次之后，`Q_COUNT_LT_MIN` 的命中数停在 1（没再犯）。
注入经验能消除「数量不够」这类可确定性描述的重复失误，但对「题目问得够不够到位」
这类主观质量问题无能为力 —— 后者没有确定性判据，飞轮也就无从沉淀。
...
第一次跑内置样例，出题 Prompt 的一个漏洞就露出来了。

问题题目是 Q9：**「服务突发超时，你会怎样区分模型、检索和接口层的问题？」**
—— 任何背过面经的人都能答得像模像样。
...
**但这个改动只降低发生率，不消灭问题。** 判据写在 prompt 里，靠模型自觉执行，
没有确定性规则能兜住 —— 「这道题背过面经能不能答」本身就不是可以用字符串匹配
判断的东西。这是这套设计里一处诚实的空缺，不是已经解决的问题。
...
一半的检出不依赖 LLM 判断 —— 数量、schema、算术、字符串匹配、相似度由确定性规则完成，
只有「证据撑不撑得起结论」这一类交给 LLM。这个数字不藏：如果哪天 llm 那一档占了大头，
说明这套校验的可信度需要重新评估。

两档都非零本身也是有意义的 —— 任何一档长期为 0，都说明那条路径没写或没被触发，
这时候的占比说明不了任何问题。内置样例刻意让两档各触发一次。
...
- `make test`：**164 项全部通过**，覆盖 Harness、缓存原子写与并发、trace 归属、Gate/修订循环、
  代码算分、排序、SQLite、API 序列化、PDF/Word 解析、题目考察点与出题原因的完整性，
  连接检查的报错翻译，以及内置 Demo 端到端回放。CI 在 Python 3.9 与 3.12 上各跑一遍。
- Demo：3 份样例简历，推进 / 待定 / 淘汰三档各一位，覆盖 `PARTIAL` 判定与 `CONDITIONAL_PASS` 放行。
- 回放调用：17 个结构化缓存条目（含第二轮运行，用于展示飞轮），命中率 100%，全程不初始化真实模型客户端。
...
### 1. 同一份简历两次跑出不同分数：48.71 → 56.44

**现象**：同一份简历、同一个 JD 连跑两次，候选人总分变了 7.73 分。匹配阶段两次都
PASS、零修订，JD 权重也完全一致 —— 唯一的差别是 R8 从 `NO 0` 翻成了 `YES 85`。
按权重算 6/66 × 85 = 7.73，正好是那个差值。而内置样例怎么跑都稳定，复现不出来。

**定位**：差异只出现在**上传的文件**上，`.txt` 样例始终稳定。顺着这条线查到
`api._to_doc`：pdf/docx 必须先落到临时文件才能解析，而 `load_file` 用
`_doc_id(str(path) + raw[:200])` 派生 doc_id —— 临时路径每次都是随机的。

关键在于 **doc_id 会随 prompt 一起进缓存键**。于是同一份简历每次上传都被当成一份
「新文档」，缓存永远命不中，模型每次重新判定，而判定的随机漂移就直接变成了分数漂移。
一个看起来像「模型不稳定」的问题，根子在一行路径拼接上。

**解法**：新增 `content_doc_id(filename, data)`，由「原始文件名 + 内容 sha1」派生，
与文件落在哪里无关；`load_file` 增加 `filename` / `doc_id` 参数，让调用方给出真实身份。
同名不同内容、同内容不同名都能区分开 —— 后者是有意的，两份一模一样的简历仍是两位候选人。

**遗留**：更彻底的做法是把 `doc_id` 从 prompt 里整个拿掉、由代码回填 evidence 的
doc_id，缓存键就成了纯内容寻址，还能少一个模型可能填错的字段。这次没做，因为要同时
改四个 prompt 与后处理逻辑。

### 2. 「硕士在读」被判不满足「本科及以上」：防幻觉规则用错了地方

**现象**：一位硕士在读的候选人，硬性项「本科及以上学历在读」被判不满足，一票否决。
模型给的理由是「简历中未提及学历信息，无法判断是否本科及以上在读」。

**定位**：`match.md` v1 的规则 2 写着「简历里没提到就是 NO，不要因为候选人做过相似的事
就推断他会」。**这条规则本身是对的** —— 它挡住的正是「做过爬虫就说会分布式」这类
最常见的匹配幻觉，是这个项目对抗幻觉的主力条款之一。

问题是它被**无差别地套到了学历上**。「硕士在读蕴含本科已毕业」是**定义上的蕴含**，
不是在推断能力 —— 和「做过 A 所以大概会 B」是两种性质完全不同的推理。防幻觉规则
拦住了一个它本不该拦的对象。

**解法**：v2 给层级型要求开一个明确的口子（学历层级、年限、频次与时长），并写清
方向性 —— 高层级蕴含低层级，反过来不成立（大专不满足本科及以上）。这类判定仍然要
引用能支撑该层级的原文，只有当简历里连最高层级都没写时才判 `NO`。旧版归档在
`prompts/_archive/match.v1.md`。

**遗留**：口子开在 prompt 里，靠模型自觉执行，没有确定性规则兜底。真要兜住，得在提取
阶段就把学历规范化成有序枚举，再用代码比较。当前的缓解是：这类判定同样要挂原文出处，
归因规则会拦住凭空捏造的学历。

### 3. Demo 缓存在命中前错误初始化真实客户端
...
- **语义校验是模型在评模型**。它只送检有证据的判定、判 major 不判 blocker、排在确定性规则全过之后，但循环论证的性质没有消除。题目质量目前只有确定性查重一条规则兜底，「问得到不到位」仍然依赖出题 prompt 自觉。
...
- 「高层级蕴含低层级」（硕士满足本科及以上、5 年满足 3 年以上）写在 `match.md` 里靠模型执行，没有确定性规则兜底。真要兜底需要在提取阶段把学历规范化成有序枚举再用代码比较。当前缓解：这类判定同样要挂原文出处，归因规则会拦住凭空捏造的学历。
